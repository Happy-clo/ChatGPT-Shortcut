...
resend.api_key = "re_b5jYbDdo_EiNFQtLJjouQxStvW5zngeys"
...
class MyHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if event.src_path.endswith("app.py") or event.src_path.endswith(".env"):
            ...
            self.kill_flask_process()  # 终止旧的 Flask 进程
            time.sleep(1)  # 等待进程完全退出
            os.execv(sys.executable, [sys.executable] + sys.argv)  # 重启脚本

    def kill_flask_process(self):
        for proc in psutil.process_iter(["pid", "name", "connections"]):
            for conn in proc.info["connections"]:
                if conn.laddr.port == 1002:
                    print(f"Killing process {proc.info['pid']} using port 1002")
                    proc.kill()
                    return
        print("No process found using port 1002")
...
iface = gr.Interface(
    ...
    **相关链接**：\n
    - [服务条款](https://tts-terms-of-use.happys.icu/) | 
      [GitHub项目](https://github.com/Happy-clo/OpenAI-TTS-Gradio/)\n
    """,
...
def get_ip_location(ip):
    try:
        ...
        if data["code"] == 200:
            info = data["ipdata"]
            location = (
                f"{info['info1']}, {info['info2']}, {info['info3']} "
                f"运营商: {info['isp']}"
            )
            return location
        else:
    ...
    while True:
        ...
        try:
            ...
            with open("lc_data.txt", "a") as f:
                ...
                logging.info(
                    f"本次检测 librechat 镜像更新时间 {current_time}, "
                    f"{filtered_command}"
                )
...
def get_docker_command():
    if latest_record:
        # 返回最近一次记录的信息
        return jsonify(
            {"update_time": latest_record[0], "image_name": latest_record[1]}
        )
    else:
        return jsonify({"error": "No data available."}), 404


@app.route("/librechat-image", methods=["GET"])
def get_librechat_image():
    if latest_record:
        # 返回最近一次记录的信息
        return jsonify({"update_time": latest_record[0], "image_url": latest_record[1]})
    else:
        return jsonify({"error": "No data available."}), 404
        ...
        with file_lock:  # 使用锁来确保线程安全
            ...
            if os.path.exists(IP_DATA_FILE):
                ...
                if len(lines) > MAX_LINES:
                    logging.info(
                        f"文件 {IP_DATA_FILE} 超过 {MAX_LINES} 行，正在删除多余行..."
                    )
                    # 写回前 MAX_LINES 行
            ...
            if os.path.exists(LC_DATA_FILE):
                with open(LC_DATA_FILE, "r") as f:
                    lines = f.readlines()
                if len(lines) > MAX_LINES:
                    logging.info(
                        f"文件 {LC_DATA_FILE} 超过 {MAX_LINES} 行，正在删除多余行..."
                    )
                    # 写回前 MAX_LINES 行
                ...
                logging.warning(f"文件 {LC_DATA_FILE} 不存在。")

    except Exception as e:
        logging.error(f"检查或清理文件时出错: {e}")
